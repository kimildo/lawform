...
class Payment extends Component {
    ...
    getDocData (iddocument) {
        API.sendGet('/documents/category/' + iddocument).then((res) => {
            ...
            IMP.request_pay(paymentOpt,
                (rsp) => {
                    ...
                    if (rsp.success) {
                        ...
                        if (param.discount < 0) param.discount = 0
                            ...
                            <div className="num">
                                {
                                    (this.state.firstPayment === true) ?
                                        <div className="">
                                            <NumberFormat value={this.state.docData.dc_price - this.state.surveyDiscount - this.state.discount} displayType={'text'} thousandSeparator={true}/>원
                                            {/* <u>첫 구매 할인 적용</u> */}
                                        </div>
                                        :
                                        <Fragment>
                                            <NumberFormat value={this.state.docData.dc_price - this.state.discount} displayType={'text'} thousandSeparator={true}/>원
                                        </Fragment>
                                }
